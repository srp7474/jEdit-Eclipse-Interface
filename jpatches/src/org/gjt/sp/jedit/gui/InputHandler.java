...
public abstract class InputHandler extends AbstractInputHandler<EditAction>
{
  ...
  /* This lets us clode the ones that cannot be reached. */

  /*public static class FixKeyEvent extends KeyEvent {
    public FixKeyEvent(Component source,int id,long when,int modifiers,int keyCode,char keyChar,int keyLocation) {
      super(source,id,when,modifiers,keyCode,keyChar,keyLocation);
    }
  }*/

  public static HashSet<String> oDropNext = new HashSet<>();

  public static KeyEvent srpIntercept(KeyEvent evt,Object oWho) {

    /* This intercept changes the numpad keycode  - and + to F23, F24 respectively
     * This allows a Brief type use of these keys for CUT and COPY by assigning
     * F23 and F24 to these values in the keymap tables.
     * The Brief mapping of numpad- and numpad+ allows for single handed CUT and PASTE
     *
     * In order to purge the system of the pending KeyChar we have to resort to this
     * hack to eliminate the next character pending when it matches what we have posted.
     *
     * srp. Aug/30/2016
     */
    //Debug.DUMP_KEY_EVENTS = true;
    if (evt.getID() == KeyEvent.KEY_TYPED) {
      String sHashDrop = ""+oWho.hashCode()+"/"+evt.getKeyChar();
      if (oDropNext.contains(sHashDrop)) {
        oDropNext.remove(sHashDrop);
        if(Debug.DUMP_KEY_EVENTS) Log.log(Log.DEBUG,oWho,"Key KILL                 : "+evt);
        return null;
      }
    } else {
      int  nKeyFlip = 0;
      char cKeyChar = 0;
      if (evt.getKeyCode() == 109) {nKeyFlip = KeyEvent.VK_F23; cKeyChar = '-';}
      if (evt.getKeyCode() == 107) {nKeyFlip = KeyEvent.VK_F24; cKeyChar = '+';}
      if (nKeyFlip != 0) {
        if(Debug.DUMP_KEY_EVENTS) Log.log(Log.DEBUG,oWho,"Key flip                 : "+evt.getKeyCode()+" to "+nKeyFlip+" "+evt.getID());
        evt.setKeyCode(nKeyFlip);
        evt.setKeyChar((char)0xFFFF);
        if (evt.getID() == KeyEvent.KEY_PRESSED) {
          String sHashDrop = ""+oWho.hashCode()+"/"+cKeyChar;
          oDropNext.add(sHashDrop);
        }
        if(Debug.DUMP_KEY_EVENTS) Log.log(Log.DEBUG,oWho,"Key now                  : "+evt);
      }
    }
    return evt;
  }
	 ...
	 */
	@Override
  public void processKeyEvent(KeyEvent evtP, int from, boolean global)
	{
    KeyEvent evt = srpIntercept(evtP,this);
    if (evt == null) {
      evtP.consume();
      return;
    }
